...
function ydot = duffing(t,y,zeta,alfa,beta,gama,We)
ydot = [y(2);gama*sin(We*t)-zeta*y(2)+alfa*y(1)-beta*y(1)^3];
end

alfa = -1.2;                       
beta = 0.3;
zeta = 0.05;                     % fator de amortecimento 
gama = 5;                       % intensidade do forçamento
We = 1;                    % frequência de excitação
y0 = [0; 0];                    % condição inicial 

fun = @(t,y)duffing(t,y,zeta,alfa,beta,gama,We);

tspan = [0, 150];                % intervalo de tempo da simulação
...
% Regime permanente
rp = round(0.95*length(T));
rp_poincare = round(0.95*size(poincare,2));  % 85% do total de pontos de Poincaré
