...
function ydot = pendulo(t,y, Wn, gama, zeta, We)
ydot = [y(2);gama*sin(We*t)-Wn^2*sin(y(1))-zeta*y(2)];
end
...
fun = @(t,y)pendulo(t,y, Wn, gama, zeta, We);

tspan = [0, 0];                % intervalo de tempo da simulação
...
% SOLUÇÃO NUMÉRICA - Passo adaptativo conforme Hairer–Wanner
...
% Regime permanente
rp = round(0.9*length(T));
rp_poincare = round(0.9*size(poincare,2));  % 85% do total de pontos de Poincaré
...
xlabel('$\theta$ (s)', 'Interpreter', 'latex');
ylabel('$\dot{\theta}$ (s)', 'Interpreter', 'latex');
